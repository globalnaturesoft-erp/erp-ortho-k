Erp::Contacts::Contact.class_eval do
  ...
  # Update cache sales debt amount
  def self.update_cache_sales_debt_amount
    self.all.each do |contact|
      contact.update_cache_sales_debt_amount
    end
    puts (Time.now.to_s + ": update all " + self.all.count.to_s)
  end
end
