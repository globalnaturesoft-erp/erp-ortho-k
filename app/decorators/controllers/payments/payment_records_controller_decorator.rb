Erp::Payments::Backend::PaymentRecordsController.class_eval do
  # Cong no theo danh sach khach hang
  ...
  # Cong no theo danh sach NCC
  def xlsx_export_supplier_liabilities
    @from = Time.now.beginning_of_month.beginning_of_day
    @to = Time.now.end_of_day
    ...
    @supplier = Erp::Contacts::Contact.find(params[:supplier_id])
    @orders = @supplier.purchase_orders.payment_for_contact_orders(params.to_unsafe_hash)
    @product_returns = @supplier.purchase_product_returns.get_deliveries_with_payment_for_contact(params.to_unsafe_hash)
    @payment_records = Erp::Payments::PaymentRecord.all_done
    ...
    respond_to do |format|
      format.xlsx {
        response.headers['Content-Disposition'] = 'attachment; filename="Bang cong no nha cung cap.xlsx"'
      }
    end
  end
  ...
  # export all customer liability details
  def export_xlsx_all_details
    ...
    send_file temp.path, type: "application/zip", x_sendfile: true,
      disposition: "attachment", filename: "ChiTietCongNoKhachHang.zip"
  end
  ...
  # export all supplier liability details
  def export_xlsx_all_supplier_details
    @from = Time.now.beginning_of_month.beginning_of_day
    @to = Time.now.end_of_day
    ...
    if glb[:salesperson_id].present?
      @suppliers = @suppliers.where(salesperson_id: glb[:salesperson_id])
    end
    ...
    @suppliers.each do |supplier|
      file_name = "#{supplier.name.to_ascii.gsub(/[^0-9a-z ]/i, '')}-#{supplier.id}.xlsx"
      create_supplier_xlsx_files(supplier, glb, tmp_path, file_name, @from, @to)
    end
    
    # zip files
    temp = Tempfile.new 'cong_no_ncc.zip'
    Zip::File.open(temp.path, Zip::File::CREATE) do |zipfile|
      @suppliers.each do |supplier|
        file_name = "#{supplier.name.to_ascii.gsub(/[^0-9a-z ]/i, '')}-#{supplier.id}.xlsx"
        zipfile.add file_name, tmp_path + file_name
    ...
    send_file temp.path, type: "application/zip", x_sendfile: true,
      disposition: "attachment", filename: "ChiTietCongNoNCC.zip"
  end
  
  def create_supplier_xlsx_files(supplier, glb, tmp_path, file_name, from, to)
    @from = from
    @to = to

    @supplier = supplier
    @orders = @supplier.purchase_orders.payment_for_contact_orders(from_date: @from, to_date: @to)
    @product_returns = @supplier.purchase_product_returns.get_deliveries_with_payment_for_contact(from_date: @from, to_date: @to)
    @payment_records = Erp::Payments::PaymentRecord.all_done
    ...
    wb.styles do |s|
      ...
      wb.add_worksheet(name: "Chi tiết công nợ") do |sheet|
        ...
        sheet.add_row ['BẢNG CHI TIẾT CÔNG NỢ (NCC)'], sz: 16, b: true, style: (s.add_style text_center)
        sheet.add_row ["(#{date})"], sz: 12, i: true, style: (s.add_style text_center)
        sheet.add_row ["Đối tượng: #{@supplier.name}"], sz: 12, b: true, style: (s.add_style text_center)
        ...
        sheet.add_row [nil]
        num_row += 1
        sheet.add_row ['1. Chi tiết mua hàng'], b: true
        num_row += 1
        ...
        if @orders.count > 0
          ...
          purchase_price_merge_num = (params[:purchase_discount_col].present? ? 1 : 0) +
            (params[:purchase_total_without_tax_col].present? ? 1 : 0) +
            (params[:purchase_tax_col].present? ? 1 : 0) +
            (params[:purchase_total_col].present? ? 1 : 0)
          
          header_1[:columns] << "Chi mua hàng"
          header_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))    
          subheader_1[:columns] << 'Đơn giá'
          subheader_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
          c += 1
          dongia = c
          sheet.merge_cells("#{('A'.codepoints.first + c).chr}#{num_row+1}:#{('A'.codepoints.first + (c+purchase_price_merge_num)).chr}#{num_row+1}")
          column_widths << 15
          
          if params[:purchase_discount_col].present?
            header_1[:columns] << nil
            header_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
            subheader_1[:columns] << 'Giảm giá'
            subheader_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
          ...
          if params[:purchase_total_without_tax_col].present?
            header_1[:columns] << nil
            header_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))    
            subheader_1[:columns] << 'Thành tiền'
            subheader_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
          ...
          if params[:purchase_tax_col].present?
            header_1[:columns] << nil
            header_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))    
            subheader_1[:columns] << 'Tiền thuế'
            subheader_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
          ...
          if params[:purchase_total_col].present?
            header_1[:columns] << nil
            header_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
            subheader_1[:columns] << 'Tổng cộng'
            subheader_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
          ...
          subheader_1[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
          c += 1
          purchase_col_note2 = c
          sheet.merge_cells("#{('A'.codepoints.first + purchase_col_note1).chr}#{num_row+1}:#{('A'.codepoints.first + purchase_col_note2).chr}#{num_row+2}")
          column_widths << 15
          ...
          @orders.each do |order|
            ...
            sheet.merge_cells("#{('A'.codepoints.first + purchase_col_note1).chr}#{num_row}:#{('A'.codepoints.first + purchase_col_note2).chr}#{num_row}")
        ...
        if @product_returns.count > 0
          ...
          header_2[:columns] << 'Khoản thu trả lại'
          header_2[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))    
          subheader_2[:columns] << 'Đơn giá'
          subheader_2[:styles] << (s.add_style bg_info.deep_merge(wrap_text).merge(bold).deep_merge(border))
          c += 1
          ...
          delivery_num_row_first = num_row + 1
          # rows //purchase imports - Product returns
          @product_returns.each do |delivery|
            row_2 = {columns: [], styles: []}
        ...
        if @payment_records.count > 0
          ...
          footer_3[:columns] << @supplier.purchase_paid_amount(from_date: @from, to_date: @to)
          footer_3[:styles] << (s.add_style bg_footer.deep_merge(number).deep_merge(bold).deep_merge(text_right).deep_merge(border))
          col_ft_3 += 1
        ...
        num_col += 1
        begin_purchase_debt_amount = @supplier.purchase_debt_amount(to_date: (@from - 1.day))
        rw1[:columns] << begin_purchase_debt_amount
        rw1[:styles] << (s.add_style text_right.deep_merge(number).deep_merge(border_dotted_bottom).deep_merge(bold))
        
        purchase_order_total_amount = @orders.sum(&:total)
        rw2[:columns] << purchase_order_total_amount
        rw2[:styles] << (s.add_style text_right.deep_merge(number).deep_merge(border_dotted_bottom))
        
        purchase_return_total_amount = @product_returns.total
        rw3[:columns] << purchase_return_total_amount
        rw3[:styles] << (s.add_style text_right.deep_merge(number).deep_merge(border_dotted_bottom))
        
        #period_amount = @supplier.purchase_total_amount(from_date: @from, to_date: @to)
        period_amount = purchase_order_total_amount - purchase_return_total_amount
        rw4[:columns] << period_amount
        rw4[:styles] << (s.add_style text_right.deep_merge(number).deep_merge(border_dotted_bottom).deep_merge(bold))
        
        purchase_paid_amount = @supplier.purchase_paid_amount(from_date: @from, to_date: @to)
        rw5[:columns] << purchase_paid_amount
        rw5[:styles] << (s.add_style text_right.deep_merge(number).deep_merge(border_dotted_bottom))
        
        #end_purchase_debt_amount = @supplier.purchase_debt_amount(to_date: @to)
        end_purchase_debt_amount = begin_purchase_debt_amount + period_amount - purchase_paid_amount
        rw6[:columns] << end_purchase_debt_amount
        rw6[:styles] << (s.add_style bg_footer.deep_merge(text_right).deep_merge(number).deep_merge(border_dotted_bottom).deep_merge(bold))
        ...
        # Column 7, 8, 9 (ĐVT, Soluong, Dongia)
        sign[:columns] << nil
        sign[:styles] << (s.add_style {})
        sign1[:columns] << nil
        sign1[:styles] << (s.add_style {})
        ...
        # Column 10, 11, 12, 13 (GG, TT, Thuế, T.cộng)
        if params[:purchase_discount_col].present? or params[:return_discount_col].present?
          sign[:columns] << nil
          sign[:styles] << (s.add_style {})
          sign1[:columns] << nil
          sign1[:styles] << (s.add_style {})
        ...
        if params[:purchase_total_without_tax_col].present? or params[:return_total_without_tax_col].present?
          sign[:columns] << nil
          sign[:styles] << (s.add_style {})
          sign1[:columns] << nil
          sign1[:styles] << (s.add_style {})
        ...
        if params[:purchase_tax_col].present? or params[:return_tax_col].present?
          sign[:columns] << nil
          sign[:styles] << (s.add_style {})
          sign1[:columns] << nil
          sign1[:styles] << (s.add_style {})
        ...
        if params[:purchase_total_col].present? or params[:return_total_col].present?
          sign[:columns] << nil
          sign[:styles] << (s.add_style {})
          sign1[:columns] << nil
          sign1[:styles] << (s.add_style {})
        ...
        sign[:columns] << nil
        sign[:styles] << (s.add_style {})
        sign1[:columns] << "NCC - #{@supplier.name}"
        sign1[:styles] << (s.add_style text_center.merge(bold))
        sign2[:columns] << '(Ký, họ tên)'
        sign2[:styles] << (s.add_style text_center.merge(italic))
        ...
        price_col_widths = []
        if params[:return_total_without_tax_col].present? or params[:purchase_total_without_tax_col].present?
          price_col_widths << 12
        end
        if params[:return_discount_col].present? or params[:purchase_discount_col].present?
          price_col_widths << 12
        end
        if params[:return_total_col].present? or params[:purchase_total_col].present?
          price_col_widths << 12
        end
        if params[:return_tax_col].present? or params[:purchase_tax_col].present?
          price_col_widths << 12
        end
